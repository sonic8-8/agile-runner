...
- 각 단계의 3개 서브에이전트 리뷰는 같은 관점을 반복하지 않고, 서로 다른 관점으로 역할을 분리해 수행한다.
- 기본 역할은 아래 3가지를 우선 사용한다.
  - 문서 경계와 범위 일관성 리뷰
  - 검증 근거와 실행 증거 충분성 리뷰
  - 운영자 가독성과 표현 자연스러움 리뷰
- task 성격에 따라 역할 이름은 바꿀 수 있지만, 3개 리뷰가 같은 질문을 반복하는 구성은 피한다.
- 서브에이전트에게는 친절한 확인보다 반례 중심의 엄격한 피드백을 요구하고, 칭찬보다 실패 가능성, 모호성, 경계 누락, 검증 빈틈을 먼저 지적하게 한다.
- 서브에이전트 리뷰 결과는 기본적으로 `PASS` 또는 `NEEDS_CHANGES`로 시작하게 고정한다.
- 서브에이전트 리뷰 결과는 최소한 `판정 1줄 + 파일 경로 기준 근거 1~3개 + 필요 시 수정점` 형식을 따른다.
