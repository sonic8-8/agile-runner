...
지금은 운영자가 실행 목록의 현재 상태 요약과 관리자 액션 history timeline을 함께 읽을 때, 두 조회가 더 자연스럽게 이어지도록 조합 기준을 정리하는 단계다.
...
- 지금 진행 중인 범위
  - 목록 조회와 관리자 액션 이력 조회 조합 정리
  - 같은 실행의 현재 상태 요약과 과거 이력을 함께 읽을 때 연결 기준을 더 분명하게 만드는 기능
- 현재 활성 spec
