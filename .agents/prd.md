...
지금은 운영자가 관리자 액션 이력에서 필요한 항목만 더 쉽게 찾도록 조회 조건을 확장하는 단계다.
...
- 지금 진행 중인 범위
  - 관리자 액션 이력 기간 필터 확장
  - 같은 실행의 관리자 액션 이력에서 필요한 기간만 골라 읽는 기능
- 현재 활성 spec
...
- 현재는 액션, action status, 기간 필터 같은 최소 조회 조건을 단계적으로 정리하는 중이며, 정렬과 페이지 기준은 다음 단계에 남아 있다.
