...
지금은 운영자가 `query`, `list`, `history`, `action` 응답을 함께 읽을 때 각 응답의 역할과 필드 의미를 더 자연스럽게 이해할 수 있도록 문서 기준을 정리하는 단계다.
...
- 지금 진행 중인 범위
  - 운영용 조회 응답 문서 기준 정리
  - 같은 execution에 대한 query, list, history, action 응답이 각각 어떤 질문에 답하는지 더 분명하게 만드는 기능
- 현재 활성 spec
