...
현재는 그 기반 위에서 운영자가 실패한 리뷰 실행을 다시 실행하고, 조회하고, 조치할 수 있는 기능 위에 운영용 문서, 기준 파일, 대표 검증 준비 데이터 유지 규칙과 적용 절차를 더 분명히 정리하는 단계다.
...
- 지금 진행 중인 범위
  ...
  - 대표 검증 전에 어떤 준비 데이터를 어떤 순서로 적용하고, schema와 enum 값은 어떤 순서로 확인할지 정리하는 기능
- 현재 활성 spec
...
## 문제
- 운영자용 조회와 조치 API, 응답 가이드, 기준 파일, 준비 데이터 파일 구조는 이미 갖춰졌지만, 실제 대표 검증 전에 무엇을 어떤 순서로 적용하고 점검해야 하는지는 아직 한 문서로 고정돼 있지 않다.
- 같은 준비 데이터를 두고도 작업자마다 적용 순서와 점검 순서가 달라지면 대표 검증 실패 원인이 제품 버그인지 준비 절차 문제인지 다시 헷갈릴 수 있다.
- schema나 enum 값을 먼저 확인하지 않고 준비 데이터를 넣으면 준비 데이터 오류가 먼저 드러날 수 있는데, 지금은 그 점검 순서와 체크리스트가 문서로 고정돼 있지 않다.
...
### 현재 단계에서 아직 남은 목표
- [ ] 준비 데이터 적용 순서와 명령 기준이 문서에서 바로 읽힌다.
- [ ] schema와 enum 값 점검 순서, 대표 검증 전 체크리스트가 문서에서 바로 읽힌다.
- [ ] 새 작업자가 준비 데이터 가이드와 준비 데이터 파일만 읽고 대표 검증 준비 절차를 따라갈 수 있다는 근거가 남는다.
...
- 운영자 조회와 조치 응답은 같은 `executionKey` 기준으로 query, list, audit evidence와 모순되지 않는다.
...
- 운영용 조회 응답 guide, 기준 파일, 자동 검증 테스트, 준비 데이터 파일 구조는 생겼지만 실제 대표 검증 전에 어떤 순서로 적용하고 무엇을 먼저 점검해야 하는지는 아직 한 문서로 정리돼 있지 않다.
- 준비 데이터 적용 절차와 대표 검증 절차를 명확히 분리해 읽는 기준이 아직 확정 단계다.
...
- 운영자 조회 기능 확장
  ...
  - 운영용 조회 응답 준비 데이터 적용 절차 정리
  - 준비 데이터 적용 명령 순서와 schema, enum 값 확인 순서 정리
  - 대표 검증 전 체크리스트 정리
