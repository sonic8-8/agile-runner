...
현재는 그 기반 위에서 운영자가 실패한 리뷰 실행을 다시 실행하고, 조회하고, 조치하고, 특히 관리자 액션 이력을 더 쉽게 찾을 수 있게 하는 기능을 확장하는 단계다.
즉, 지금 단계의 제품 목표는 `자동 리뷰 기능 자체를 더 많이 늘리는 것`보다 `운영자용 조회와 조치 기능을 더 정교하게 다듬는 것`에 더 가깝다.
...
지금은 운영자가 `query`, `list`, `history`, `action` 응답을 함께 읽을 때 각 응답의 역할과 필드 의미를 더 자연스럽게 이해할 수 있도록 정리한 문서 기준을, 이후 코드 변경에도 자동으로 검증되게 고정하는 단계다.
...
- 지금 진행 중인 범위
  - 운영용 조회 응답 예시 자동 검증
  - 같은 execution에 대한 query, list, history, action 응답 예시가 이후 코드 변경으로 조용히 어긋나지 않게 자동 검증 기반을 만드는 기능
- 현재 활성 spec
...
## 문제
- 자동 리뷰 실행, 수동 재실행, 결과 조회, 관리자 액션 같은 운영 기능은 이미 갖춰졌지만, 운영자가 필요한 실행과 이력을 빠르게 찾기에는 검색성과 조회 조건이 아직 충분하지 않다.
- 실패 실행의 상태, 조치 이력, 다음 가능한 액션은 남기고 있지만, 운영자가 현재 상태와 과거 이력을 한 번에 읽기에는 조회 관점이 더 정리될 필요가 있다.
...
### 현재 단계에서 아직 남은 목표
- [ ] 운영자가 관리자 액션 이력을 필요한 조건으로 좁혀 읽을 수 있는 최소 필터 경계가 정리된다.
- [ ] 운영자 조회 기능이 현재 상태 요약과 과거 이력 읽기를 계속 분리해서 유지한다.
...
- 현재는 목록 현재 상태 요약과 관리자 액션 history timeline을 더 자연스럽게 함께 읽도록 조회 조합 기준을 정리하는 중이다.
...
- 운영자 조회 기능 확장
  ...
  - 관리자 액션 이력 기간 필터 확장
  - 운영자가 필요한 이력 항목만 빠르게 읽을 수 있는 조회 조건 정리
  - 현재 상태 요약과 과거 이력 읽기 경계 유지
  - 운영용 조회 응답 예시 자동 검증
  - 문서 예시와 현재 응답 계약 drift 조기 감지
- 운영 안정성 잔여 과제
