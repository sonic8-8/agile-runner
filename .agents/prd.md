...
즉, 지금 단계의 제품 목표는 `자동 리뷰 기능 자체를 더 많이 늘리는 것`보다 `운영자용 조회와 조치 기능을 반복 검증하는 절차를 실제 스크립트 초안 범위까지 좁혀 검토하는 것`에 더 가깝다.
...
지금은 운영자가 단건 조회, 목록 조회, 이력 조회, 관리자 조치 응답을 함께 검증할 때 쓰는 대표 검증 절차를 어디까지 문서로 유지하고, 어디까지 반자동 스크립트로 묶을지 정리하는 단계다.
...
- 지금 진행 중인 범위
  ...
  - 대표 검증 절차의 반복 비용과 스크립트 후보 단계 정리
  - 수동 실행 근거 확인 단계와 자동화 후보 단계의 경계 정리
- 현재 활성 단계
...
- 반대로 이 절차를 전부 자동화하면 실행 흔적 가시성, 오류 분리 지점, H2 lock 대응 같은 운영 근거가 흐려질 수 있어, 어디까지를 스크립트로 묶을지 기준이 아직 없다.
...
- [ ] 대표 검증 절차에서 스크립트 후보 단계와 수동 실행 근거 확인 단계를 문서로 구분할 수 있다.
- [ ] 대표 검증 스크립트 도입 이점, 위험, 유지 비용, 비대상이 문서로 정리된다.
- [ ] 후속 스크립트 초안 검토로 넘어갈지 여부가 마감 근거와 함께 남는다.
...
- 운영자 조회 기능 확장
  ...
  - 대표 검증 절차에서 실제 초안으로 묶을 명령 범위와 입력/출력 파일 경계 정리
  - 초안이 멈춰야 하는 실패 조건과 계속 수동으로 남길 확인 단계 정리
  - 준비 데이터, 기준 파일, 대표 검증, 보조 명령의 경계 유지
- 운영 안정성 잔여 과제
