...
즉, 지금 단계의 제품 목표는 `자동 리뷰 기능 자체를 더 많이 늘리는 것`보다 `운영자용 조회와 조치 기능을 반복 검증하는 절차를 실제 스크립트 초안 파일 구조까지 좁혀 검토하는 것`에 더 가깝다.
...
지금은 운영자가 단건 조회, 목록 조회, 이력 조회, 관리자 조치 응답을 함께 검증할 때 쓰는 대표 검증 절차를 어떤 초안 파일 구조로 반자동화할지 검토하는 단계다.
...
- 지금 진행 중인 범위
  ...
  - 대표 검증 절차의 초안 파일 구조, 입력/출력 계약, 인계 지점 정리
  - 수동 실행 근거 확인 단계와 초안 파일 후보 단계의 경계 정리
- 현재 활성 단계
  - `SPEC-0029 운영용 조회 응답 준비 데이터 반복 검증 스크립트 초안 구현 검토`
- 직전 완료 단계
  - `SPEC-0028 운영용 조회 응답 준비 데이터 반복 검증 스크립트 초안 검토`
- 다음 후보 단계
...
- [ ] 대표 검증 절차에서 스크립트 초안 파일 후보 구조와 수동 실행 근거 확인 단계를 문서로 구분할 수 있다.
- [ ] 대표 검증 스크립트 도입 이점, 위험, 유지 비용, 비대상이 문서로 정리된다.
- [ ] 후속 스크립트 초안 구현 단계로 넘어갈지 여부가 마감 근거와 함께 남는다.
- [ ] 현재 구현 단계와 남은 확장 방향이 PRD에서 바로 읽히는 상태를 유지한다.

## 성공 지표
- 동일 webhook 재처리 회귀 시나리오에서 중복 본문 코멘트가 발생하지 않는다.
- 성공 또는 실패한 `WebhookExecution(실행 기록)`은 상태, `ValidationCriteria(검증 기준)`, `Execution Log(단계별 실행 로그)`를 빠짐없이 남긴다.
- 로컬 및 테스트 환경의 기본 컨텍스트 기동은 외부 secret 없이 재현 가능하다.
- 실패한 webhook 처리 흐름은 어느 단계에서 실패했는지 한 번에 식별 가능하다.
- 운영자 조회와 조치 응답은 같은 `executionKey` 기준으로 단건 조회, 목록 조회, 감사 근거와 모순되지 않는다.
...
- 운영자 조회 기능 확장
  ...
  - 운영용 조회 응답 준비 데이터 반복 검증 스크립트 초안 구현 검토
  - 대표 검증 절차에서 실제 초안 파일 후보 구조와 입력/출력 계약 정리
  - 초안 종료 흐름과 계속 수동으로 남길 확인 단계 정리
  - 준비 데이터, 기준 파일, 대표 검증, 보조 명령, 초안 파일 후보의 경계 유지
- 운영 안정성 잔여 과제
...
- 하나의 `Task`는 검증 과정에서 0개 이상의 `WebhookExecution` 실행 근거를 가질 수 있다.
