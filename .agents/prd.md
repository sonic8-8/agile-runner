...
즉, 지금 단계의 제품 목표는 `자동 리뷰 기능 자체를 더 많이 늘리는 것`보다 `운영자용 조회와 조치 기능을 반복 검증하는 절차를 어디까지 반자동으로 묶을지`를 더 명확하게 다듬는 것에 더 가깝다.
...
- 운영자 조회 기능 확장
  ...
  - 운영용 조회 응답 준비 데이터 반복 검증 스크립트 검토
  - 대표 검증 절차에서 스크립트 후보 단계와 수동 실행 근거 확인 단계 구분
  - 스크립트 도입 이점, 위험, 유지 비용 정리
