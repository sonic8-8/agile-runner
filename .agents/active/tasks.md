...
| `TASK-0002` | 종료 코드 실패 사례 빠른 참조 정리 | 종료 코드 10~33 실패 유형을 짧은 표로 먼저 좁히게 정리 | `manual-rerun-script-stop-code-quick-reference-documented` | 대상 테스트 + 전체 테스트 + 표만 보고 먼저 열 로그를 고를 수 있는지 확인 | 새 이슈 |
...
| `TASK-0004` | 빠른 참조 마감과 상세 문서 연결 정리 | 빠른 참조를 본 뒤 다음에 열 상세 예시 표와 보조 문서, 마지막에 다시 확인할 질문 정리 | `manual-rerun-script-quick-reference-closeout-documented` | 대상 테스트 실행 통과 + 저장소 표준 전체 테스트 명령 통과 + 문서 마감 리뷰 통과 | 새 이슈 |
...
- 표만 보고 먼저 열 로그와 먼저 좁힐 실패 유형을 다시 고를 수 있다는 리뷰 근거가 회고에 남는다.
...
- 카드만 보고 먼저 펼칠 카드와 먼저 열 로그 또는 파일, 먼저 던질 질문을 다시 고를 수 있다는 리뷰 근거가 회고에 남는다.
...
### 목표
- 빠른 참조를 본 뒤 어느 상세 예시 표와 보조 문서를 열어야 하는지, 마지막에 무엇을 다시 확인해야 하는지 문서로 정리한다.

### 구현 범위
- 아래 기준을 문서에 반영한다.
  - 빠른 참조 뒤에 열 상세 예시 표와 보조 문서 연결 정리
...
### 완료 조건
- 빠른 참조를 본 뒤 다음에 열 상세 예시 표와 보조 문서, 마지막에 다시 확인할 질문이 문서로 남는다.
