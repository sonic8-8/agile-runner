...
## 이 문서와 보조 명령 가이드의 역할 경계
- 이 문서는 `어떤 파일을 고르고 어떤 순서로 다루는지`를 설명한다.
- `manual-rerun-response-seed-command-guide.md`는 `어떤 명령을 어떤 변수로 실행하는지`를 설명한다.
- 즉 이 문서는 입력 자료 선택과 적용 순서 쪽 기준이고, 보조 명령 가이드는 실제 명령 실행과 출력 근거 자료 확보 쪽 기준이다.
- 다음 단계에서 스크립트 후보를 검토할 때도, 파일 선택과 순서 판단은 이 문서를 기준으로 보고, 실제 명령 후보와 수동 확인 단계 경계는 보조 명령 가이드를 기준으로 본다.
