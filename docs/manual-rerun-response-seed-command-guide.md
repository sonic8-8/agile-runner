...
적용 예시 정리를 마감할 때는 아래 순서로 다시 확인한다.

### 유지 보수 체크리스트
1. 먼저 이번 변경이 `적용 예시 문장`, `상세 예시 표`, `응답 가이드`, `출력 파일 이름`, `대표 검증 근거 문장` 중 어디에 걸리는지 정한다.
2. 변경 대상에 맞춰 이 문서의 `적용 예시를 고칠 때 먼저 여는 문서`와 `변경 시 함께 갱신해야 하는 문서와 기준 파일`에서 같이 열 문서와 기준 파일을 다시 고른다.
3. 적용 예시와 상세 예시 표가 어긋났다면 `출력 파일 이름 변경과 문서 어긋남 감지 기준` 순서대로 적용 예시, 상세 예시 표, 응답 가이드 또는 마지막 대표 검증 회고를 다시 본다.
4. 출력 파일 이름이나 경로 설명을 실제로 바꿨다면 스크립트 파일과 스크립트 테스트를 먼저 다시 보고, 문서 표현만 바뀐 경우와 섞지 않는다.
5. 관련 대상 테스트를 먼저 순차 실행해 자동 검증 기대값이 아직 맞는지 확인한다.
6. 전체 `cleanTest test`를 순차 실행해 문서 정리 작업이 기존 자동 검증과 충돌하지 않는지 확인한다.
7. 실제 앱/H2 대표 검증이 이번 변경 범위가 아니라면 비대상 사유를 회고에 남긴다.
8. 마지막으로 회고, 최신 포인터, 현재 단계 요약 문서를 같이 갱신한다.

### 마지막으로 다시 확인할 질문
- 이번 변경이 적용 예시와 상세 예시 표를 같은 실패 상황 기준으로 다시 묶었는가.
- 출력 파일 이름 변경과 문서 어긋남을 서로 다른 문제로 유지했는가.
- 응답 가이드, 마지막 대표 검증 회고, 현재 단계 요약 문서가 같은 실행과 같은 출력 파일을 설명하는가.
- 대상 테스트와 전체 `cleanTest test` 결과가 회고와 최신 포인터에 반영됐는가.

### 단계 마감 기준
- 적용 예시 유지 기준 단계는 아래가 모두 맞아야 마감으로 본다.
  - 먼저 볼 문서와 같이 갱신할 파일이 문서에 남아 있다.
  - 어긋남 점검 순서가 문서에 남아 있다.
  - 마지막 확인 질문과 체크리스트가 문서에 남아 있다.
  - 관련 대상 테스트와 전체 `cleanTest test`가 순차 실행으로 통과한다.
  - 실제 앱/H2 대표 검증이 비대상이면 그 이유가 회고에 남아 있다.
  - 현재 단계 요약 문서와 최신 포인터가 현재 상태를 가리킨다.
...
## 적용 예시를 본 뒤 실제 사용 순서
1. 지금 보고 있는 실패가 종료 코드, 출력 파일 누락, H2 조회 실패 중 어디에서 먼저 좁혀졌는지 정한다.
2. 해당 카드나 표에서 바로 연결된 상세 예시 표로 내려가, 실제로 먼저 볼 로그나 파일을 다시 확인한다.
3. 상세 예시 표를 읽은 뒤에도 응답 의미 비교가 필요하면 `응답 의미 비교 참고`를 함께 연다.
4. 마지막 대표 검증에서 같은 유형을 어떻게 닫았는지 보고 싶으면 `마지막 대표 검증 비교 참고`를 함께 연다.
5. 마지막으로 위 `빠른 참조 마감 전 마지막 확인 질문`을 다시 보고, 여전히 같은 실패 유형으로 읽히는지 확인한다.

## 이 단계 마감 기준
- 종료 코드, 출력 파일 누락, H2 조회 실패 중 어디에서 먼저 시작해야 하는지 문서만 보고 다시 고를 수 있어야 한다.
- 각 카드나 표에서 다음에 열 상세 예시 표와 참고 문서를 문서 안에서 바로 찾을 수 있어야 한다.
- 응답 의미 비교가 필요한 경우와, H2 결과를 사람이 직접 판단해야 하는 경우가 문서에서 섞이지 않아야 한다.
- 문서만 보고 마지막 확인 질문까지 다시 따라갈 수 있어야 한다.
